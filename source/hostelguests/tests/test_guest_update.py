...
class GuestUpdateTestCase(TestCase):
    def setUp(self) -> None:
        ...
        self.permission = Permission.objects.get(codename='can_change_guest')
        self.guest = GuestFactory(photo=SimpleUploadedFile('test.jpg', content=b'', content_type='image/jpg'))

    def tearDown(self) -> None:
        self.client.logout()

    def test_unauthorized_get_update_guest(self):
        response = self.client.get(reverse('hostelguests:guest_update', kwargs={'pk':self.guest.pk}))
        redirect_url = reverse('accounts:login') + '?next=' + reverse('hostelguests:guest_update', kwargs={'pk':self.guest.pk})
        self.assertRedirects(response, redirect_url, status_code=302)

    def test_unauthorized_post_update_guest(self):
        response = self.client.post(reverse('hostelguests:guest_update', kwargs={'pk':self.guest.pk}))
        redirect_url = reverse('accounts:login') + '?next=' + reverse('hostelguests:guest_update', kwargs={'pk':self.guest.pk})
    ...
    def test_authorized_without_permission_get_update_guest(self):
